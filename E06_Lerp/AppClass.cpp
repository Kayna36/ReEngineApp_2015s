...
void AppClass::InitVariables(void)
{
	...
	//float fPercent = MapValue(static_cast<float>(nSphere), 0.0f, static_cast<float>(m_nObjects), 0.0f, 1.0f); ask later
		float fPercent = MapValue(static_cast<float>(i), 0.0f, static_cast<float>(m_nObjects)-1, 0.0f, 1.0f);
	//m_pSphere->GenerateSphere(1.0f, 5, vector3(fPercent, 0.0f, 0.0f)); that was to the whole array woops
		m_pSphere[i].GenerateSphere(.5f, 5, RERED);
		vector3 tanslating = glm::lerp(v3Start, v3End, fPercent);
	//m_pMatrix[i] =  glm::translate(vector3(fPercent, 0.0f, 0.0f));
		m_pMatrix[i] = glm::translate(tanslating);
	}
/*
